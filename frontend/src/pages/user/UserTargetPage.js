...
function UserTargetPage() {
  ...
  return (
    <div className={cx('wrapper')}>
      <h2 className={cx('title')}>Mục tiêu của tôi</h2>
      ...
      {targetScore && partRequirements.length > 0 && (() => {
        ...
        const color =
          diff === 0 ? 'var(--success-text)' : diff > 0 ? 'var(--primary)' : '#d97706';
        const bg =
          diff === 0 ? 'var(--success-bg)' : diff > 0 ? '#eff6ff' : '#fffbeb';
        return (
        ...
        <div className={cx('currentTarget')}>
          ...
          <div className={cx('currentTargetParts')}>
            {(currentTarget.partRequirements || []).map((p) => {
              ...
              return (
                <span key={p.examPartId} className={cx('partBadge')}>
      ...
      {message && (
        <p className={cx('message', { success: isSuccess, error: !isSuccess })}>{message}</p>
      )}
    </div>
  );
}
