...
import { useAttemptCertificate } from '~/features/certificate/hooks/useCertificates';
...
function CertificateBanner({ userTestId, enabled }) {
  ...
  if (!data || data.state === 'NOT_APPLICABLE') {
    return null;
  }

  if (data.state === 'NOT_PASSED') {
    return (
  ...
  const justIssued = data.state === 'JUST_ISSUED';
