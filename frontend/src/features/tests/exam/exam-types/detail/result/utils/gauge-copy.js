// Toàn bộ câu chữ hiển thị cho card kết quả (gauge + "Việc cần làm ngay").
// BE chỉ trả facts (percentage, readinessLevel, targetScore...), FE quyết định copy tại đây.

// Bậc đánh giá Quick Challenge — duyệt từ cao xuống, lấy bậc đầu tiên đạt min.
// Không có message: breakdown tag sai + nút Full Mock ngay dưới card đã là hành động cụ thể.
 ...
 * Trả về { gaugePercentage, displayValue, gaugeLabel, gaugeTitle, gaugeMessage, gaugeLevel }.
 */
export function buildGaugeView(enhanced) {
  ...
  const copy = READINESS_GAUGE_COPY[readinessLevel] || READINESS_GAUGE_COPY.NOT_READY;
  return {
    gaugePercentage: score,
    displayValue: `${score}%`,
    gaugeLabel: 'Độ sẵn sàng',
    gaugeTitle: copy.title,
