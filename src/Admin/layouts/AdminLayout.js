...
const adminRoutes = [
    ...
    { path: routes.adminClasses, icon: <GraduationCap size={20} />, label: 'Quản lý Lớp học' },
    { path: routes.adminTests, icon: <BookOpen size={20} />, label: 'Quản lý Bài thi' },
    { path: routes.adminQuestions, icon: <FileQuestion size={20} />, label: 'Quản lý Câu hỏi' },
    { path: routes.adminVocabulary, icon: <BookMarked size={20} />, label: 'Quản lý Từ vựng' },
    ...
    return (
        <div className={cx('adminWrapper')}>
            ...
            <>
                ...
                {/* Sidebar */}
                <aside className={cx('sidebar', { collapsed, 'mobile-open': mobileOpen })}>
                    {/* Logo */}
                        ...
                        <div className={cx('navSection')}>
                            ...
                            {adminRoutes.map((route) => (
                                ...
                                >
                                    ...
                                    {!collapsed && <span className={cx('navText')}>{route.label}</span>}
                                    {isActive(route.path) && (
                                        <motion.div
                                            layoutId="activeNav"
                    ...
                    <div className={cx('bottomSection')}>
                        ...
                        >
                            <span className={cx('navIcon')}><LogOut size={20} /></span>
                            {!collapsed && <span className={cx('navText')}>Về trang chủ</span>}
                        </Link>
                    </div>
                </aside>
            </>
