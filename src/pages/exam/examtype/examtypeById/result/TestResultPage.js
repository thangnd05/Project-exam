    ...
    <div className={`question-result ${resultClass}`}>
      ...
      {question.questionType === "MCQ" && (
        <>
          {question.answers?.map((a) => {
            ...
            return (
              <div key={a.answerId}
                className={`answer ${a.isCorrect ? 'correct-answer' : ''} ${isUserSelected && !a.isCorrect ? 'incorrect-choice' : ''}`}>
                <span className="answer-text">{a.answerLabel}. {a.answerText}</span>
                <div className="answer-tags">
                  {a.isCorrect && <span className="tag correct-tag">✅ Đáp án đúng</span>}
                  {isUserSelected && !a.isCorrect && <span className="tag incorrect-tag">🟡 Bạn chọn</span>}
                  {isUserSelected && a.isCorrect && <span className="tag your-choice-tag">🟢 Bạn chọn & đúng</span>}
                </div>
              </div>
            );
          })}
      ...
      {question.questionType === "FILL_BLANK" && (
        <div className="fill-blank-result">
          ...
          {!isUserCorrect && correctAnswer && (
            <p><strong>Đáp án đúng: </strong> <span className="correct-text">{correctAnswer.answerText}</span></p>
          )}
        </div>
      )}
      ...
      {question.questionType === "ESSAY" && (
        <div className="essay-result">
          ...
          <div className="essay-content">{userAnswer?.answerText || "(Chưa trả lời)"}</div>
        </div>
      )}
...
function TestResultPage() {
  ...
  return (
    <div className="test-result-page">
      ...
      {score !== null && (
        <p className="score"><strong>Điểm của bạn: {score.toFixed(2)}</strong></p>
      )}
        ...
        <div key={part.testPartId} className="test-part">
          ...
          {part.passage && (
            <div className="passage">
              <h4>{part.passage.passageType === "LISTENING" ? "Bài nghe:" : "Đoạn văn:"}</h4>
