...
const QuestionBankCreator = () => {
  ...
  const [questions, setQuestions] = useState([
    {
      ...INITIAL_QUESTION,
      answers: INITIAL_QUESTION.answers.map((a) => ({...a})),
    },
  ]);
    ...
    try {
      ...
      if (mode === MODES.SINGLE) {
        const payload = {
          ...
          ...questions[0],
          passage:
            passage.content || passage.mediaUrl || audioFile ? passage : null,
        };
        ...
        if (audioFile) {
          ...
          singleForm.append(
            ...
            JSON.stringify({
              ...payload,
              passage: {...passage, passageType: 'LISTENING'},
            }),
          );
    ...
    <div className={cx('wrapper')}>
      <div className={cx('container')}>
        ...
        <div className={cx('modeSelector')}>
          ...
          <button
            className={cx('modeBtn', {
              active: mode === MODES.BULK_INDEPENDENT_AUDIO,
            })}
        ...
        <div className={cx('formSection')}>
          <h3>
            <IoSchoolOutline /> Thông tin chung
          </h3>
        ...
        {(mode === MODES.SINGLE || mode === MODES.BULK_PASSAGE) && (
          <div className={cx('formSection')}>
            <h3>
              <IoBookOutline /> Shared Passage (Nội dung chung)
            </h3>
            <div className={cx('inputGroup')}>
              ...
              <select
                ...
                onChange={(e) =>
                  setPassage({...passage, passageType: e.target.value})
                }
              >
        ...
        <div className={cx('formSection')}>
          <h3>
            <IoLayersOutline /> Danh sách câu hỏi ({questions.length})
          </h3>
            ...
            <div key={qIndex} className={cx('questionItem')}>
              ...
              {mode === MODES.BULK_INDEPENDENT_AUDIO && (
                <div className="mb-3 p-3 bg-light rounded-3 border">
                  <label className="fw-bold mb-2 d-flex align-items-center gap-2">
                    <IoMusicalNotesOutline className="text-primary" /> Audio
                    riêng cho câu này
                  </label>
                  <input
                    type="file"
              ...
              <div className={cx('answerGrid')}>
                {q.answers.map((a, aIndex) => (
                  <div
                    key={aIndex}
                    className={cx('answerItem', {correct: a.isCorrect})}
                  >
                    <input
                      type="radio"
                      name={`q-${qIndex}`}
