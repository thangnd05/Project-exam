        ...
        <Container fluid="lg">
              ...
              {user ? (
                <>
                  <Nav.Link as={Link} to={routes.createTest} className={cx("mx-5")}>
                    <Button variant="" className={cx("new-test")}>Tạo bài kiểm tra</Button>
                  </Nav.Link>
                  <Dropdown>
                    <Dropdown.Toggle as="div" className={cx("user-info")}>
                      <Image src={images.avtImage} alt="Avatar" className={cx("avatar")} />
                      <div><span className={cx("username")}>{user.username}</span></div>
                    </Dropdown.Toggle>
                    <Dropdown.Menu className={cx("custom-dropdown")}>
                      <Dropdown.Item as={Link} to={routes.profile}>Hồ sơ</Dropdown.Item>
                      <Dropdown.Item onClick={handleLogout} as={Link} to={routes.home}>Đăng xuất</Dropdown.Item>
                    </Dropdown.Menu>
                  </Dropdown>
                </>
              ) : (
          ...
          <Offcanvas show={showOffcanvas} onHide={handleClose} placement="end">
            <Offcanvas.Header closeButton>
              <Offcanvas.Title as={Link} to={routes.home} onClick={handleClose}>EDT</Offcanvas.Title>
            </Offcanvas.Header>
            <Offcanvas.Body>
              <Nav className="flex-column" onClick={handleClose}>
                <Nav.Link as={Link} to={routes.content}>Bài viết</Nav.Link>
                <Nav.Link as={Link} to={routes.about}>Giới thiệu</Nav.Link>
                {user ? (
                  <>
                    <Nav.Link as={Link} to={routes.post}>Tạo bài viết</Nav.Link>
                    <Dropdown>
                      <Dropdown.Toggle as="div" className={cx("user-info")}>
                        <Image src={images.avtImage} alt="Avatar" className={cx("avatar")} />
                        <div><span className={cx("username")}>{user.username}</span></div>
                  ...
                  <>
                    <Nav.Link as={Link} to={routes.login}>Đăng nhập</Nav.Link>
                    <Nav.Link as={Link} to={routes.register}>Đăng ký</Nav.Link>
                  </>
                )}
              </Nav>
            </Offcanvas.Body>
